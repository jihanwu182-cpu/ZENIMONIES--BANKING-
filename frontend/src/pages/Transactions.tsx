...
const Transactions: React.FC = () => {
  ...
  const formatAmount = (
    ...
    try {
      return new Intl.NumberFormat(
        selectedCurrency === 'ZAR' ? 'en-ZA' : 'en-NG',
        {
          style: 'currency',
  ...
  const getDescription = (transaction: Transaction) => {
    ...
    return String(transaction.type || 'Transaction')
      .replace(/_/g, ' ')
  ...
  const getRelativeDate = (date?: string) => {
    ...
    const isYesterday =
      parsed.toDateString() === yesterday.toDateString();
  ...
  const getTransactionIcon = (
    ...
    if (transactionCategory === 'transfers') {
    ...
    if (transactionCategory === 'airtime') {
      return <AirtimeRounded />;
    }

    if (transactionCategory === 'bills') {
      return <BillRounded />;
    }
  ...
  const getIconColors = (transaction: Transaction) => {
    const transactionCategory =
      getCategory(transaction);
  ...
  const moneyOut = useMemo(() => {
    return filteredTransactions
      .filter(
        (transaction) => !isCredit(transaction)
      )
      .reduce(
        (total, transaction) =>
          total + Number(transaction.amount || 0),
  ...
  const copyReference = async (
    ...
    try {
      ...
      console.log('Unable to copy reference.');
    }
  };

  const selectedStatus = selectedTransaction
  ...
  return (
    <Box
      sx={{
        minHeight: '100vh',
        background:
          'linear-gradient(180deg, #F3FAF7 0px, #F7F9F8 310px)',
        pb: 7,
      }}
    >
      {/* =====================================================
